...
from PySide6.QtGui import QAction, QActionGroup, QIcon, QFont, QFontMetricsF, QFontMetrics, QTextCharFormat
...
from dicts import amhara_dict, yi_dict, kantaiji_dict, hieroglyph_dict, cuneiform_dict

from key_events import amhara_method, yi_method, bopomofo_method,kantaiji_method, hieroglyph_method, cuneiform_method
...
class TextEdit(QTextEdit):
    ...
    def __init__(self, window, parent=None):
        ...
        self.kantaiji_dic = kantaiji_dict.dic
        self.kantaiji_keys = self.kantaiji_dic.keys()
    ...
    def get_text(self):
        ...
        if self.listwidget.isVisible():
            if self.split_text == [""] or not self.split_text:
                self.listwidget.setHidden(True)

        self.extraSelectionsUpdate()

    def padding(self):
        count = len(self.henkan_after_list)%9
        if count:
            for i in range(9-count, 0, -1):
                self.henkan_after_list.append("")
                
                
    def re_search(self):

        tc = self.textCursor()
        if not self.isPinyinEditting:
            self.start_pos = tc.position() -1
            self.isPinyinEditting = 1
        ...
        alphabet = re.compile("[*]", re.MULTILINE)
        self.split_text = alphabet.split(tc.selectedText())
        text = self.split_text[0] + "*"

        for i in self.kantaiji_keys:
            if i.match(text):
                kanjis = self.kantaiji_dic[i]
                self.henkan_after_list = list(kanjis)

        self.extraSelectionsUpdate()
        self.padding()
        self.listwidget.items = [
            QListWidgetItem() for i in range(
                0,
                len(self.henkan_after_list))
            ]
        for l, kanji in enumerate(self.henkan_after_list):

            if kanji:
                self.listwidget.items[l].setText(str(l%9+1)+kanji)
            else:
                self.listwidget.items[l].setText(kanji)

        for number, kantaiji in enumerate(
                                self.listwidget.items
                                ):
            self.listwidget.insertItem(number, kantaiji)

            if self.isPinyinEditting:
    ...
    def keyPressEvent(self, event):
        ...
        elif self.writingSystem == zh_CN:
           
            kantaiji_method.proxy_keyPressEvent(self, event)
            return

        elif self.writingSystem == hi_EG:
            hieroglyph_method.proxy_keyPressEvent(self, event)
            return
        elif self.writingSystem == cu_PE:
            cuneiform_method.proxy_keyPressEvent(self, event)
            return

        elif self.writingSystem == gez_ET:
            amhara_method.proxy_keyPressEvent(self, event)
            return

        elif self.writingSystem == km_KH:

            if event.keyCombination() == Key_Control_F3:
                self.key_page = not self.key_page
                self.changeWritingSystem(self.key_page)
                return
    ...
    def repairing_insertion(self, tc, insert_text):

        tc.insertText(insert_text)
        self.end_pos = max(tc.position(), self.end_pos)

    def new_insertion(self, tc, insert_text,s):

        tc.clearSelection()
        tc.setPosition(s, MoveAnchor)
        tc.insertText(insert_text)
        self.end_pos = max(tc.position(), self.end_pos)
        
        

    def changeWritingSystem(self, page=0):
        ...
        if self.writingSystem == ko_KR:
            ...
            self.document().setDefaultFont(QFont("游明朝", 18))
            self.isPinyinEditting = 0
            self.split_text = []
            self.henkan_after_list = []
            self.listwidget.items = []
            self.listwidget_font = QFont('游明朝', 20)
            self.henkan_num = 0
            self.start_pos = 0
            self.end_pos = 0
            self.font_metrics = QFontMetrics(self.listwidget.font())
            self.listwidget.setWritingSystem(zh_CN)

        elif self.writingSystem == hi_EG:
            self.insert_dic = KeyDict()
            self.insert_dic += hieroglyph_dict.dic
            self.insert_dic_keys = self.insert_dic.keys()
            self.listwidget.setWritingSystem(hi_EG)

            tc = self.textCursor()
            char = tc.charFormat()
            font = QFont('Segoe UI Historic', 24)
            char.setFont(font)
            tc.setCharFormat(char)
            self.setTextCursor(tc)
            font = QFont('Segoe UI Historic', 12)
            for button in self.keyDicts.values():
                button.setFont(font)

        elif self.writingSystem == cu_PE:
            self.henkan = 0
            self.insert_dic = KeyDict()
            self.insert_dic += cuneiform_dict.dic
            self.insert_dic_keys = self.insert_dic.keys()

            self.listwidget.setWritingSystem(self.writingSystem)
            self.listwidget_font = QFont("Segoe UI Historic", 20)
            self.font_metrics = QFontMetrics(self.listwidget.font())
            tc = self.textCursor()
            char = tc.charFormat()
            font = QFont("Segoe UI Historic", 24)
            ...
            tc.setCharFormat(char)
            self.setTextCursor(tc)
            amhara_dict.initAmhara(self)
            self.sIsPushed = False
            self.hIsPushed = False

        elif self.writingSystem == km_KH:
            font = QFont("Khmer UI", 24)
            tc = self.textCursor()
            char = tc.charFormat()
            char.setFont(font)
            tc.setCharFormat(char)
        ...
        with sqlite3.connect(db_path) as con:
            ...
            cur = con.cursor()
            cur.execute(f"SELECT * FROM {self.writingSystem} WHERE PAGE = ?", (page,))
            dic = {}
            data = cur.fetchall()
            for key in data:
                dic[key[0]] = key
